...
function renderResponsive(md, tokens, idx, options, env, self, patterns) {
  // normal fields
  var filename = md.utils.escapeHtml(tokens[idx].attrs[0][1]);
  var src = ' src="' + filename + '"';
  var title = '';
  if (tokens[idx].attrs[2]) {
    title = ' title="' + md.utils.escapeHtml(tokens[idx].attrs[2][1]) + '"';
  }
  var alt = ' alt="' + md.utils.escapeHtml(tokens[idx].content) + '"';
